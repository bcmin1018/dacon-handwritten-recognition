...
def run(input_path, output_path):
    ...
    for idx, item in enumerate(files):
        ...
        #gt = train_df['label'][idx] 
        #gt = os.path.basename(item).split('_')[0]   # remove index
        img_idx = gt = os.path.basename(item).split('_')[1].split('.')[0]
        gt = train_df['label'][int(img_idx)]
        ext = os.path.splitext(item)[1]
        filename = os.path.join(image_folder_name, ('image_%0{}d'.format(digits) % int(img_idx)) + ext)
        gt_file.write('%s\t%s\n' % (filename, gt))
